...
class MoveableEntity
{
  ...
  /* Board contains the game map */
  boolean[][] Board;

  /* gridSize is the size of one square in the game.
     max is the height/width of the game.
     speed is the speed at which the object moves,
     1 speed per move() call */
  int gridSize;
  int max;
  int speed;
  ...
  public boolean isValidDest(int x, int y)//makes sure coordinates fall within the play area
  {
    if ((((x) % 20 == 0) || ((y) % 20) == 0) && 20 <= x && x < 400 && 20 <= y && y < 400 && Board[x/20-1][y/20-1] )
      return true;
...
{
  /* Direction is used in currDirection and desiredDirection are used in non demoMode*/ 
  char direction;
  char currDirection;
  ...
  /* The move function moves the pacman one frame */
  public void move()
  {
    int gridSize=20;
    lastX=x;
    ...
    /* Otherwise, clear the stopped flag and speed the frameCount for animation purposes*/
    else
    {
      stopped=false;
      frameCount ++;
...
class Ghost extends MoveableEntity
{ 
  ...
  /*Constructor places ghost and updates Boards*/
...
/*This Map class contains the player, ghosts, pellets, and most of the game logic.*/
public class Map extends JPanel
{
  ...
  */
  ...
  /* Contains the Board of all pellets*/
  boolean[][] pellets;
  ...
  /* Constructor initializes Board flags etc.*/
  public Map() 
  {
    initHighScores();
    sounds = new Sounds();
  ...
  public void reset()
  {
    ...
    /* Clear Board and pellets arrays */
    for(int i=0;i<20;i++)
    {
      for(int j=0;j<20;j++)
      {
  ...
  */
  ...
  /*  This function draws the Map.  The pacman Map is really complicated and can only feasibly be done
  ...
  public void paint(Graphics g)
  {
    ...
    if (dying > 0)
    {
      ...
      /* Draw the pacman */
      g.drawImage(pacmanImage,player.x,player.y,Color.YELLOW,null);
      g.setColor(Color.BLACK);
    ...
    if (New==1)
    {
      ...
      player.Board[9][7]=false; 
      ghost1.updateBoard(Board);
      ghost2.updateBoard(Board);
      ghost3.updateBoard(Board);
      ghost4.updateBoard(Board);
    ...
    {
      /* Stay in this Board until the sound effect is over */
      long currTime = System.currentTimeMillis();
    ...
    if ( pellets[player.pelletX][player.pelletY] && New!=2 && New !=3)
    {
      ...
      /* speed pellets eaten value to track for end game */
      player.pelletsEaten++;
